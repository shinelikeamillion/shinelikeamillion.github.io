    ...
    function ($) {
        ...
        $.setup = $.draw = _ => {
            ...
            for (y = 100; y < b; y += 5) {
                $.beginShape()
                for (x = 0; x < b; ++x)
                    $.vertex(x, y - 80 / (1 + $.pow(x - 150, 4) / 8e6) * $.noise(x / 30 + f / 50 + y))
                // 可以不用作者这个 1 * 10000
                // v(x,1e4)
                $.endShape()
            }
    ...
    function ($) {
        ...
        $.draw = _ => {
            ...
            for (let i = 0; i <= size; i++) {
                upY = $.map($.sin($.frameCount / 40 + i / 120 * $.HALF_PI), -1, 1, h, h + 40)
                downY = $.map($.sin($.frameCount / 20 + i / 120 * $.HALF_PI), -1, 1, upY, upY + 15)
                // 保证此点的 Y 轴在圆内
